...
#This is a comment
'''
This will get you in trouble instead
'''
...
d
d <- 3.14159L # does not work w decimal value
```
...
```{r}
country <- 'France'
city_country <- paste(city, country)
city_country

sprintf('%s is the city and %s is the country', city, country)
```
...
```{r}
student_name <- c('James', 'Andy', 'Sid')
student_grade_levels <- c('Junior', 'Sophomore', 'Sophomore')
...
1.  Create an array of two 3x3 matrices from the given vectors.
 ...
 #first_array_of_matrices <- array(
  #c(1, 2, 3, 4, 5, 6, 7, 8, 9),
  #dim=c(3,3,1)
#)
#first_array_of_matrices

 #second_array_of_matrices <- array(
  #c(10, 11, 12, 13, 14, 15, 16, 17, 18),
  #dim=c(3,3,1)
#)
#second_array_of_matrices
    
    #single line:
    array_of_matrices <- array(c(vec1, vec2), dim = c(3, 3, 2))
    array_of_matrices
...
2.  In the above matrix that you created, print the value 5 by indexing in to the matrix.
    ...
    array_of_matrices[2,2,1]
    ```

3.  Print the third row of the first matrix.

    ```{r}
    array_of_matrices[3,,1]
    ```
...
6.  What values can a BOOLEAN contain?
    ...
    ```{r}
#Booleans can only contain True or False values
    ```
